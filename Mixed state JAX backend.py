...
def generate_mixed_state(n_qubits, like, rank=None, seed=None):
    """
    Generate a random n-qubit mixed state using the backend of ``like``.

    The state is sampled with a complex Ginibre matrix A:
    ...
    rho = ginibre @ xp.conj(matrix_transpose(ginibre, xp))
